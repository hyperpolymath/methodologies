...
Three-axis methodology applied in strict order: *Scope → Maintenance →
Audit*. Each axis has its own priority ordering. Followed by a
cleanup/finish-off phase and mandatory maintainer dialogue.
...
*Inputs:* - README, roadmap, status docs, CI/security docs - Marker
scan: `+TODO+`, `+FIXME+`, `+XXX+`, `+HACK+`, `+STUB+`, `+PARTIAL+` -
...
|*Adaptive* |Scope reconciliation, stale-reference removal,
...
Maintainer dialogue is *REQUIRED* at the end. Topics: - What changed -
Why - Remaining risks
...
|Best for |Audits, releases, compliance |Exploration, migration,
integration
|===
